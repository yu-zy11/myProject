...
void testForwardKinematics() {
  // std::vector<std::string> end_effector_names = {"aa", "bb", "cc"};
  // std::string name = "cc";
  // auto it = std::find(end_effector_names.begin(), end_effector_names.end(), name);
  // if (it == end_effector_names.end()) {
  //   printf("Error, end effector name: [%s] does not exits in end_effector_datas, please check\n", name.c_str());
  //   abort();
  // }
  // int index = std::distance(end_effector_names.begin(), it);
  ...
  //   if (data[i].pos[0] != data_full[i].pos[0] || data[i].pos[1] != data_full[i].pos[1] || data[i].pos[2] !=
  //   data_full[i].pos[2]) {
  //     success = false;
  //   }
  //   if (data[i].vel[0] != data_full[i].vel[0] || data[i].vel[1] != data_full[i].vel[1] || data[i].vel[2] !=
  //   data_full[i].vel[2]) {
  //     success = false;
  //   }
  // }
