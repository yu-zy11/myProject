...
class RobotState:
    ...
    def update_user(self):
        if self.use_joy:
            ...
            self.gait_type = 0
        # print("self.gait_type = ", self.gait_type)
    ...
    def update_counter(self):
        ...
        for i in range(4):
            if self.foot_state[i] == FOOT_ST:
                self.foot_counter_st[i] += self.foot_counter_st_speed[i]
            elif self.foot_state[i] == FOOT_SW:
                self.foot_counter_sw[i] += self.foot_counter_sw_speed[i]
    ...
    def draw_lines(self):
        ...
        root_est_project_pose.p = gymapi.Vec3(*self.root_pos_des_abs)
...
# load asset
asset_root = "/home/ubuntu/software/model_files"
try:
    asset_name = sys.argv[1]
except IndexError:
    asset_name = "px2_et1"
    pass
asset_file = "px2_et1/urdf/" + asset_name + ".urdf"
...
calf_name = ["fl3_l", "fr3_l", "hl3_l", "hr3_l"]
calf_index = [rigid_body_dict[name] for name in calf_name]

foot_name = ["fl_foot", "fr_foot", "hl_foot", "hr_foot"]
